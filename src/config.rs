...
/// status checks on HTML routes). Must be reachable from this pod, unlike
/// `identity_public_base_url`, which is the browser-facing ingress host and
/// does not resolve back to identity from inside the cluster network. Falls
/// back to the public URL for non-cluster local dev.
#[must_use]
